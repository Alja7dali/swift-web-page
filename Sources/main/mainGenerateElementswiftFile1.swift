...
// //     "Document",
// //     """
// //     /// The `<!doctype>` element represents a html document.
// //     """,
// //   ], [
// //     "Dt",
// //     """
...
// // //     public let tag: StaticString = \"\(tag[0].lowercased())\"
// // //     public let attributes: Array<Attribute>
// // //     public let content: HtmlNode = EmptyNode()
...
// // ///public init<N: HtmlNode>\(\n    @NodeBuilder _ content: \(\) -> N\n  \) where N: (.*) \{\n   self.init\(\[\], content\(\)\)\n  \}
// // ///public init<N: HtmlNode>(@NodeBuilder _ c: () -> N) where N: $1 {\n    attributes = []; content = c()\n  }




// // let path = "/Users/halllo/Desktop/swift-dom/Sources/Html/Aliases.swift"

// // try? File(path: path).append("/// MARK: - AlternativeAliases\n\n")

// // allTags.filter({ $0.count == 3 }).sorted(by: { $0[0] < $1[0] }).forEach {
// //   try? File(path: path).append("public typealias \($0[2]) = \($0[0])\n")
// // }


// // try? File(path: path).append("/// MARK: - LowercasedAliases\n\n")

// // allTags.sorted(by: { $0[0] < $1[0] }).forEach {
// //   try? File(path: path).append("public typealias \($0[0].lowercased()) = \($0[0])\n")
// // }


// // try? File(path: path).append("/// MARK: - AlternativeLowercasedAliases\n\n")

// // allTags.filter({ $0.count == 3 }).sorted(by: { $0[0] < $1[0] }).forEach {
// //   try? File(path: path).append("public typealias \($0[2].lowercased()) = \($0[0])\n")
// // }
